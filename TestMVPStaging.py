...
def click_test(css_element,hoteltrial,trial,log_file):
    try:
        ...
        test_result = "OK"
        print(f"{hoteltrial}.{trial} - Testando agora id: {id} - component: {component}, RESULT: {test_result}")
...
for id in giata:
    if current_test < number_of_tests:
        line += 1
        current_test += 1
        hotel_url = f"https://tui-lte:tuiLTE!@tuilte-guidence-br.stg.silversurfer7.com/detail/goal:{id}?from=20200324&to=20200327&adults=2&utm_source=TripAdvisor"
        print(f"New test! Hotel {id}, test number = {current_test} - {hotel_url} ")
        driver.set_page_load_timeout(10)
        driver.get(f"https://tui-lte:tuiLTE!@tuilte-guidence-br.stg.silversurfer7.com/detail/goal:{id}?from=20200324&to=20200327&adults=2&utm_source=TripAdvisor")
